...
class Sun extends CelestialBody {
    constructor(config) {
        ...
        this.corona.scale.set(this.size * 3, this.size * 3, 1);
        this.group.add(this.corona);
    }
    update(elapsedTime, delta){ 
        this.group.rotation.y += delta * 0.02;
        this.corona.material.rotation += delta * 0.01;
        this.corona.scale.setScalar(this.size * 3 * (1 + Math.sin(elapsedTime * 0.5) * 0.05));
    }
}
...
class Planet extends CelestialBody {
    constructor(config) {
        ...
        let material;
        if (this.name === "Кузня") {
            material = new THREE.MeshStandardMaterial({ map: this.textures?.map, roughness: 0.8, emissiveMap: this.textures?.map, emissive: 0xffffff, emissiveIntensity: 0.6 });
        } else if (this.name === "Пакт") {
...
// --- ЗАПУСК ВСЕСВІТУ З ПЕРЕВІРКОЮ WEBGL ---
if ((() => { try { const canvas = document.createElement('canvas'); return !!window.WebGLRenderingContext && (canvas.getContext('webgl') || canvas.getContext('experimental-webgl')); } catch(e) { return false; } })()) {
    try {
        new Universe();
