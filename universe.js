// Архітектурне ядро Всесвіту. Версія 3.1. ДІАГНОСТИЧНИЙ РЕЖИМ.
...
console.log('[ARCHITECT] Ядро завантажено. Починаю діагностику.');

function initializeUniverse() {
    console.log('[ARCHITECT] initializeUniverse() викликано.');

    // --- Увесь попередній код класів та констант залишається тут ---
    ...
    const PLANET_DATA = [ { name: "Світ 1", size: 10, distance: 220, speed: 0.1, textureUrl: 'https://cdn.prod.website-files.com/687800cd3b57aa1d537bf6f3/687e6b9829eb2d0c5a1d1a83_1.1.png', hasMoon: true, description: "Опис для Світу 1...", url: "#" }, { name: "Світ 2", size: 8, distance: 280, speed: 0.08, textureUrl: 'https://cdn.prod.website-files.com/687800cd3b57aa1d537bf6f3/687e6b95e0b0e78f91b89f0e_2.1.png', description: "Опис для Світу 2...", url: "#" }, { name: "Світ 3", size: 12, distance: 340, speed: 0.06, textureUrl: 'https://cdn.prod.website-files.com/687800cd3b57aa1d537bf6f3/687e6b96486c5b3d3b30f732_3.1.png', hasRings: true, description: "Опис для Світу 3...", url: "#" }, { name: "Світ 4", size: 9, distance: 400, speed: 0.05, textureUrl: 'https://cdn.prod.website-files.com/687800cd3b57aa1d537bf6f3/687e6b95f5194ad7643a12c6_4.1.png', hasMoon: true, description: "Опис для Світу 4...", url: "#" }, { name: "Світ 5", size: 15, distance: 480, speed: 0.04, textureUrl: 'https://cdn.prod.website-files.com/687800cd3b57aa1d537bf6f3/687e6b924dddf1f2a16788df_5.1.png', description: "Опис для Світу 5...", url: "#" }, { name: "Світ 6", size: 7, distance: 560, speed: 0.035, textureUrl: 'https://cdn.prod.website-files.com/687800cd3b57aa1d537bf6f3/687e6b981b847f548b44f1f5_6.1.png', description: "Опис для Світу 6...", url: "#" }, { name: "Світ 7", size: 11, distance: 640, speed: 0.03, textureUrl: 'https://cdn.prod.website-files.com/687800cd3b57aa1d537bf6f3/687e6b95eac3b87509fde04f_7.1.png', description: "Опис для Світу 7...", url: "#" }, { name: "Світ 8", size: 13, distance: 730, speed: 0.025, textureUrl: 'https://cdn.prod.website-files.com/687800cd3b57aa1d537bf6f3/687e6b94b53ba90dbc022678_8.1.png', hasMoon: true, description: "Опис для Світу 8...", url: "#" }, { name: "Світ 9", size: 6, distance: 820, speed: 0.022, textureUrl: 'https://cdn.prod.website-files.com/687800cd3b57aa1d537bf6f3/687e6b92e0b0e78f91b89af5_9.1.png', description: "Опис для Світу 9...", url: "#" }, { name: "Світ 10", size: 10, distance: 910, speed: 0.02, textureUrl: 'https://cdn.prod.website-files.com/687800cd3b57aa1d537bf6f3/687e6b93f5194ad7643a11b9_10.1.png', description: "Опис для Світу 10...", url: "#" }, { name: "Світ 11", size: 9, distance: 1000, speed: 0.018, textureUrl: 'https://cdn.prod.website-files.com/687800cd3b57aa1d537bf6f3/687d0eb009d11e7ccc1190bc_%D0%BF%D0%BB%D0%B0%D0%BD%D0%B5%D1%82%D0%B0%201.png', description: "Опис для Світу 11...", url: "#" }, { name: "Світ 12", size: 14, distance: 1100, speed: 0.016, textureUrl: 'https://cdn.prod.website-files.com/687800cd3b57aa1d537bf6f3/687e6b9829eb2d0c5a1d1a83_1.1.png', description: "Опис для Світу 12 (текстура-заглушка)...", url: "#" }, { name: "Світ 13", size: 12, distance: 1210, speed: 0.014, textureUrl: 'https://cdn.prod.website-files.com/687800cd3b57aa1d537bf6f3/687c024e724b8fbdbee74a65_The%20Supreme%20Ink%20Dynasty.png', hasRings: true, description: "Опис для Світу 13...", url: "#" }, { name: "Світ 14", size: 8, distance: 1320, speed: 0.012, textureUrl: 'https://cdn.prod.website-files.com/687800cd3b57aa1d537bf6f3/687e6b9829eb2d0c5a1d1a83_1.1.png', hasMoon: true, description: "Опис для Світу 14 (текстура-заглушка)...", url: "#" } ];
    class UIManager {
        constructor(planets, cameraManager) { 
            console.log('[ARCHITECT] UIManager: Починаю ініціалізацію.');
            this.sidebar = document.getElementById('sidebar');
            console.log('[ARCHITECT] UIManager: Знайдено #sidebar ->', this.sidebar);
            this.infoPanel = document.getElementById('planet-info'); this.planetName = document.getElementById('planet-name'); this.planetDesc = document.getElementById('planet-desc'); this.planetLink = document.getElementById('planet-link'); this.planets = planets; this.cameraManager = cameraManager; if (this.sidebar) this.populateSidebar(); else console.error('[ARCHITECT] UIManager: #sidebar НЕ ЗНАЙДЕНО. Меню не буде створено.');
        }
        populateSidebar() { console.log('[ARCHITECT] UIManager: Заповнюю меню.'); let listItems = ''; this.planets.forEach(planet => { listItems += `<li data-planet-name="${planet.name}">${planet.name}</li>`; }); this.sidebar.innerHTML = `<div class="sidebar-header"><h1>Книга Мандрівника</h1></div><div class="divider"></div><div id="nav-content"><ul>${listItems}</ul></div>`; this.sidebar.querySelectorAll('li').forEach(li => { li.addEventListener('click', (e) => { const planetName = e.target.getAttribute('data-planet-name'); const targetPlanet = this.planets.find(p => p.name === planetName); if (targetPlanet) { this.cameraManager.focusOn(targetPlanet); } }); }); }
    ...
    class CameraManager {
        constructor(container, camera) {
            console.log('[ARCHITECT] CameraManager: Починаю ініціалізацію.');
            console.log('[ARCHITECT] CameraManager: Отримано контейнер ->', container);
            this.uiManager = null; this.focusedObject = null; this.isAnimating = false; this.camera = camera; this.container = container; this.initialPosition = this.camera.position.clone(); this.initialFov = this.camera.fov; this.orbitControls = new OrbitControls(this.camera, this.container); this.flyControls = new FlyControls(this.camera, this.container); this.configureOrbitControls(); this.configureFlyControls(); this.activeControls = this.orbitControls; this.flyControls.enabled = false; this.orbitControls.addEventListener('start', () => this.onManualControlStart());
        }
        configureOrbitControls() { this.orbitControls.enableDamping = true; this.orbitControls.dampingFactor = 0.05; this.orbitControls.autoRotate = false; this.orbitControls.minPolarAngle = 0; this.orbitControls.maxPolarAngle = Math.PI; this.orbitControls.minDistance = 150; this.orbitControls.maxDistance = 5000; this.orbitControls.screenSpacePanning = false; }
        configureFlyControls() { this.flyControls.movementSpeed = 800; this.flyControls.rollSpeed = Math.PI / 10; this.flyControls.autoForward = false; this.flyControls.dragToLook = false; this.flyControls.lookSpeed = 0.1; }
        onManualControlStart() { if (this.isAnimating) { gsap.killTweensOf(this.camera.position); gsap.killTweensOf(this.orbitControls.target); gsap.killTweensOf(this.camera); this.isAnimating = false; } this.focusedObject = null; this.orbitControls.autoRotate = false; }
        setUIManager(uiManager) { this.uiManager = uiManager; }
        update(delta) { if (this.focusedObject && !this.isAnimating && this.activeControls === this.orbitControls) { const targetPosition = new THREE.Vector3(); this.focusedObject.group.getWorldPosition(targetPosition); this.orbitControls.target.lerp(targetPosition, 0.1); } this.activeControls.update(delta); }
        onResize() { this.camera.aspect = window.innerWidth / window.innerHeight; this.camera.updateProjectionMatrix(); }
        setMode(mode) { if (mode === 'fly') { this.activeControls = this.flyControls; this.flyControls.enabled = true; this.orbitControls.enabled = false; this.orbitControls.autoRotate = false; this.focusedObject = null; if (this.uiManager) this.uiManager.hidePlanetInfo(); console.log("РЕЖИМ ПОЛЬОТУ: Активовано."); } else { this.activeControls = this.orbitControls; this.flyControls.enabled = false; this.orbitControls.enabled = true; console.log("РЕЖИМ ОГЛЯДУ: Активовано."); } }
        focusOn(targetObject) { if (this.isAnimating || this.focusedObject === targetObject) return; this.setMode('orbit'); this.isAnimating = true; this.focusedObject = targetObject; this.orbitControls.autoRotate = false; if (this.uiManager) this.uiManager.showPlanetInfo(targetObject); const targetPosition = new THREE.Vector3(); targetObject.group.getWorldPosition(targetPosition); const endPosition = new THREE.Vector3(targetPosition.x + targetObject.size * 4, targetPosition.y + targetObject.size * 2, targetPosition.z + targetObject.size * 4); const tl = gsap.timeline({ onComplete: () => { this.isAnimating = false; this.orbitControls.minDistance = targetObject.size * 1.2; this.orbitControls.maxDistance = targetObject.distance * 1.5; this.orbitControls.target.copy(targetPosition); } }); tl.to(this.camera.position, { duration: 2.5, x: endPosition.x, y: endPosition.y, z: endPosition.z, ease: 'power3.inOut' }, 0); tl.to(this.orbitControls.target, { duration: 2.5, x: targetPosition.x, y: targetPosition.y, z: targetPosition.z, ease: 'power3.inOut' }, 0); tl.to(this.camera, { duration: 2.5, fov: 35, onUpdate: () => this.camera.updateProjectionMatrix(), ease: 'power3.inOut' }, 0); }
        resetFocus() { if (this.isAnimating) return; this.setMode('orbit'); this.isAnimating = true; this.focusedObject = null; if (this.uiManager) this.uiManager.hidePlanetInfo(); const tl = gsap.timeline({ onComplete: () => { this.isAnimating = false; this.orbitControls.autoRotate = false; this.orbitControls.minDistance = 150; this.orbitControls.maxDistance = 5000; } }); tl.to(this.camera.position, { duration: 2.5, ...this.initialPosition, ease: 'power3.inOut' }, 0); tl.to(this.orbitControls.target, { duration: 2.5, x: 0, y: 0, z: 0, ease: 'power3.inOut' }, 0); tl.to(this.camera, { duration: 2.5, fov: this.initialFov, onUpdate: () => this.camera.updateProjectionMatrix(), ease: 'power3.inOut' }, 0); }
    }
    ...
    class Universe {
        constructor() {
            console.log('[ARCHITECT] Universe: Починаю конструктор.');
            this.container = document.getElementById('webgl-canvas');
            if (!this.container) {
                console.error("[ARCHITECT] Universe Constructor: #webgl-canvas НЕ ЗНАЙДЕНО. Зупиняю ініціалізацію.");
                return;
            }
            this.scene = new THREE.Scene(); this.clock = new THREE.Clock(); this.celestialBodies = []; this.interactiveObjects = []; this.raycaster = new THREE.Raycaster(); this.mouse = new THREE.Vector2(); this.hoveredPlanet = null; this.init();
        }
        async init() {
            console.log('[ARCHITECT] Universe: Починаю init().');
        ...
        addEventListeners() { 
            ...
            window.addEventListener('mousemove', (e) => { 
                ...
                if (customCursor) gsap.to(customCursor, { duration: 0.3, x: e.clientX, y: e.clientY });
            }, false); 
    ...
    // Механізм "Гарантований Запуск"
    const requiredIds = ['webgl-canvas', 'sidebar', 'custom-cursor'];
    let ready = false;
    const checker = setInterval(() => {
        console.log('[ARCHITECT] Перевірка готовності DOM...');
        const canvas = document.getElementById('webgl-canvas');
        const sidebar = document.getElementById('sidebar');
        const cursor = document.getElementById('custom-cursor');
        console.log(`[ARCHITECT] -> #webgl-canvas: ${canvas ? 'OK' : 'NULL'}, #sidebar: ${sidebar ? 'OK' : 'NULL'}, #custom-cursor: ${cursor ? 'OK' : 'NULL'}`);
        
        ready = requiredIds.every(id => document.getElementById(id) !== null);
        if (ready) {
            console.log('[ARCHITECT] DOM готовий. Запускаю Всесвіт.');
