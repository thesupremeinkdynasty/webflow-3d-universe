// Архітектурне ядро Всесвіту. Версія 3.2. Протокол "Вільний Орієнтир".
import * as THREE from 'three';
import { TrackballControls } from 'three/addons/controls/TrackballControls.js'; // Змінено
...
function initializeUniverse() {
    // Весь код класів Sun, Planet, CelestialBody, CometCursor та константи залишаються без змін
    ...
    class CameraManager {
        constructor(container, camera, renderer) { // Додано renderer
            ...
            // --- ОСНОВНА ЗМІНА: TrackballControls ---
            this.controls = new TrackballControls(this.camera, renderer.domElement);
            this.configureControls();
            
            this.container.addEventListener('mousedown', () => this.onManualControlStart());
        }
        configureControls() {
        ...
        update() { // Delta більше не потрібна для TrackballControls
        ...
        focusOn(targetObject) {
            ...
            const endPosition = new THREE.Vector3(
                targetPosition.x + targetObject.size * 4,
                targetPosition.y + targetObject.size * 2,
                targetPosition.z + targetObject.size * 4
            );
            ...
            tl.to(this.controls.target, { duration: 2.5, x: targetPosition.x, y: targetPosition.y, z: targetPosition.z, ease: 'power3.inOut' }, 0);
        ...
        resetFocus() {
            ...
            tl.to(this.controls.target, { duration: 2.5, x: 0, y: 0, z: 0, ease: 'power3.inOut' }, 0);
    ...
    class Universe {
        ...
        animate() {
            ...
            this.cameraManager.update(); // Змінено
