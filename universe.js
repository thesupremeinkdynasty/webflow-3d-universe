// Архітектурне ядро Всесвіту. Версія 3.6. Протокол "Надійний Обробник Подій".
...
function initializeUniverse() {
    // Класи та константи залишаються без змін...
    ...
    class UIManager {
        ...
        showPlanetInfo(planetData) {
            if (!this.infoPanel || !this.planetName || !this.planetDesc) return;
            
            const planetLinkButton = document.getElementById('planet-link');
            if (!planetLinkButton) return;

            this.planetName.textContent = planetData.name;
            this.planetDesc.textContent = planetData.description;
            
            // Створюємо клон кнопки, щоб видалити всі попередні обробники подій
            const newButton = planetLinkButton.cloneNode(true);
            planetLinkButton.parentNode.replaceChild(newButton, planetLinkButton);

            // Додаємо новий обробник подій через addEventListener
        ...
        hidePlanetInfo() { if (this.infoPanel) this.infoPanel.classList.remove('visible'); }
    }
    ...
    class CameraManager {
        ...
        onManualControlStart() { if (this.isAnimating) { gsap.killTweensOf(this.camera.position); gsap.killTweensOf(this.controls.target); gsap.killTweensOf(this.camera); this.isAnimating = false; } this.focusedObject = null; }
        setUIManager(uiManager) { this.uiManager = uiManager; }
        update(delta) { this.controls.update(delta); }
        onResize() { this.camera.aspect = window.innerWidth / window.innerHeight; this.camera.updateProjectionMatrix(); this.controls.handleResize(); }
    ...
    const checker = setInterval(() => {
        ...
        if (ready) {
            ...
            try { new Universe(); } catch (e) { console.error("Критична помилка під час ініціалізації Всесвіту:", e); }
        }
    }, 100);
}

try { initializeUniverse(); } catch(e) { console.error("Помилка запуску:", e); }// Архітектурне ядро Всесвіту. Версія 3.6. Протокол "Надійний Обробник Подій".
import * as THREE from 'three';
import { TrackballControls } from 'three/addons/controls/TrackballControls.js';
import { EffectComposer } from 'three/addons/postprocessing/EffectComposer.js';
import { RenderPass } from 'three/addons/postprocessing/RenderPass.js';
...
try { initializeUniverse(); } catch(e) { console.error("Помилка запуску:", e); }
